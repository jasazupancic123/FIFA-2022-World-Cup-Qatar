...
module.exports = class GoalController {
  static async find(req, res, next) {
    try {
      ...
      for (let i = 0; i < goals.length; i++) {
        ...
        goals[i].scorer.nationality = await TeamModel.findById(goals[i].scorer.nationality)
        goals[i].scorer.club = await ClubModel.findById(goals[i].scorer.club)
        goals[i].assister = assister
        goals[i].assister.nationality = await TeamModel.findById(goals[i].assister.nationality)
        goals[i].assister.club = await ClubModel.findById(goals[i].assister.club)
        goals[i].match = match
  ...
  static async findById(req, res, next) {
    try {
      ...
      const awayManager = await ManagerModel.findById(awayTeam.manager)
      goal.scorer = scorer
      goal.scorer.nationality = await TeamModel.findById(scorer.nationality)
      goal.scorer.club = await ClubModel.findById(scorer.club)
      goal.assister = assister
      goal.assister.nationality = await TeamModel.findById(assister.nationality)
      goal.assister.club = await ClubModel.findById(assister.club)
      goal.match = match
